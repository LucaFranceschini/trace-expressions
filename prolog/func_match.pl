...
parse_json_pre(json(O), Event, Name, Id, Args, ArgIds, TargetId) :-
    parse_json_pre(json(O), Event, Name, Id, Args, TargetId),
    member(argIds=ArgIds, O).
...
func_post(Json, Name, Args, Res, ResultId) :- parse_json_post(Json,
	func_post, Name, Args, Res, ResultId).
