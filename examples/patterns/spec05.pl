...
:- module(spec,[trace_expression/2, match/2, zero/1]).
...
trace_expression('test', filter >>  app(Main,42)) :- %%% end must occur only if all 42 bytes have been consumed
    Main = gen(size,guarded(zero(var(size)),end:eps,var(newSize,consume(var(size),var(newSize)):app(Main,var(newSize))))).
