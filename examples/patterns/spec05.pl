...
:- module(spec,[trace_expression/2, match/2, gtzero/1]).

:- use_module(node(func_match)).

match(Json, filter) :- match(Json, end).
...
match(Json, end) :- func_pre_name(Json , 'end').
...
trace_expression('test', filter >>  app(Main,42)) :- %%% end must occur only if all 42 bytes have been consumed
    Main = gen(size,guarded(gtzero(var(size)),var(newSize,consume(var(size),var(newSize)):app(Main,var(newSize))),end:eps)).
