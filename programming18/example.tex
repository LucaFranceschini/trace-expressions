...
After that, the file is closed, and a callback printing \lstinline{'bye'} is provided.
...
From the documentation, it turns out that a second write to the same file should only be called \emph{after} the callback of the previous one has been executed.
...
\tau_w &= \var{id_2}{\writet(\avar{id_2}) \prefixop \cbt(\avar{id_2}) \prefixop \tau'} \orop \tau_c\\
